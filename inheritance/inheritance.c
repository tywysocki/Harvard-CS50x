...
int main(void)
{
    // Seed random number generator (we want to be able to randomly choose which gene gets passed on from parent to child)
    srand(time(0));

    // Create a new family with three generations (returns a pointer with the most recent generation of person)
    person *p = create_family(GENERATIONS);

    // Print family tree of blood types
    print_family(p, 0);

    // Free memory (frees the memory allocated during the creation of each family) - use malloc and then free
    free_family(p);
}
